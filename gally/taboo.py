...
from gally.bot import is_channel
...
class TabooExt:
    ...
    #             ))
    #         else:
    #             await self.bot.say("The card {} does not exists in the database.".format(card))
    #
    ...
    #
    #         cursor.execute("select value from settings where setting like 'SECONDS'")
    #         seconds = int(cursor.fetchone())
    #
    #         cursor.close()
    #         connection.close()
    #
    #         if channel.id == 'NONE':
    #             await self.bot.say("Can't start a new game. A channel hasn't been set.")
    #             return
    #
